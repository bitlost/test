...
echo $secure->validate($passports);
...
Class Security {
  ...
  var $valid = 0;
  var $byr, $iyr, $eyr, $hgt, $hcl, $ecl, $pid, $cid;
  
  function validate($passports) {  
  ...
  function isValid($passport) {
    ...
    if ($this->isValidBirthYear($passport) && 
        $this->isValidIssueYear($passport) && 
        $this->isValidExpirationYear($passport) && 
        $this->isValidHeight($passport) && 
        $this->isValidHairColor($passport) && 
        $this->isValidEyeColor($passport) && 
        $this->isValidPassportId($passport) && 
        $this->isValidCountryId($passport)) {
      return true;
    }
    return false;
  }
  ...
  function isValidBirthYear($passport) {
    preg_match('/byr:\s?([0-9]{4})\s?/', $passport, $matches);

    if (empty($matches)) return false;

    if (intval($matches[1]) < 1920 || intval($matches[1]) > 2002) return false;

    $this->byr = $matches[1];
    return true;
  }

  function isValidIssueYear($passport) {
    preg_match('/iyr:\s?([0-9]{4})\s?/', $passport, $matches);

    if (empty($matches)) return false;

    if (intval($matches[1]) < 2010 || intval($matches[1]) > 2020) return false;
  ...
  function isValidExpirationYear($passport) {
    preg_match('/eyr:\s?([0-9]{4})\s?/', $passport, $matches);

    if (empty($matches)) return false;

    if (intval($matches[1]) < 2020 || intval($matches[1]) > 2030) return false;
  ...
  function isValidHeight($passport) {
     preg_match('/hgt:\s?([0-9]+)(cm|in)\s?/', $passport, $matches);

    if (empty($matches)) return false;

    if ($matches[2] == 'cm' && (intval($matches[1]) < 150 || intval($matches[1]) > 193)) return false;

    if ($matches[2] == 'in' && (intval($matches[1]) < 59 || intval($matches[1]) > 76)) return false;  

    $this->hgt = $matches[1] . $matches[2];
    return true;
  }

  function isValidHairColor($passport) {
    preg_match('/hcl:\s?(#[0-9|a-f]{6})\s?/', $passport, $matches);
  ...
  function isValidEyeColor($passport) {
    preg_match('/ecl:\s?(amb|blu|brn|gry|grn|hzl|oth)\s?/', $passport, $matches);
  ...
  function isValidPassportId($passport) {
    preg_match('/pid:\s?([0-9]{9})\s?/', $passport, $matches);
  ...
  function isValidCountryId($passport) {  
    return true;
  }
}
