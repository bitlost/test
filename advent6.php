...
$custom_input = explode("\n\n", $input);

$customs = new CustomDeclaration($custom_input);

echo "Unique: " . $customs->getUniqueGroupAnswers() . "\n";
echo "Group: " . $customs->getGroupMatchAnswers() . "\n";
...
class CustomDeclaration {
  ...
  function getUniqueGroupAnswers() {
    $count = 0;
    
    foreach($this->input as $answers) {
      $count += count($this->getUniqueAnswers($answers));
    }
    
    return $count;
  }
  
  private function getUniqueAnswers($answers) {
   return array_unique(str_split(preg_replace("#\r\n|\r|\n|\n\r#", "", $answers)));
  }
  
  function getGroupMatchAnswers() {
    $count = 0;
    
    foreach ($this->input as $answers) {
      $individual_answers = explode("\n", $answers);
      $size_of_group = count($individual_answers);
      $unique_answers = $this->getUniqueAnswers($answers);
      
      foreach ($unique_answers as $answer) {
        $count = (substr_count($answers, $answer) == $size_of_group) ? 
          $count + 1 : $count;
      }
    }
