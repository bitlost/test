...
$input = "plaid magenta bags contain 2 clear lavender bags, 3 clear teal bags, 4 vibrant gold bags.
...
$sample_input = "shiny gold bags contain 2 dark red bags.
...
$luggage = new LuggageProcessing($input);
echo "\n\n" . $luggage->getBagsInsideColor('shiny gold');
...
class LuggageProcessing {
  ...
  function __construct($input) {
    $this->input = $input;
    $this->formatted_input = $this->getFormatedInput();
  }
  
  private function getFormatedInput() {
    $formatted_input = array();
    
    foreach($this->input as $input) {
      
      preg_match('/(.*) bags contain (.*)/', $input, $matches);
      
      $outer_bag = $matches[1];
      $inner_bags = array();
      
      foreach (explode(",", $matches[2]) as $bags) {
        preg_match('/(\d) (.*) bag?/', $bags, $inner_matches);
        
        if(empty($inner_matches)) continue;
        
        $inner_bags[] = array('num' => $inner_matches[1], 'color' => $inner_matches[2]);
      }
      
      $formatted_input[$outer_bag] = $inner_bags;
    }    
    return $formatted_input;
  }
  
  public function getHowManyBagsContain($color) {
    $counter = count($this->getBagsThatHoldColor($color));
    return $counter;
  }
  
  private function getBagsThatHoldColor($color) {
    
    $this->checked_colors[] = $color;
    $matched_colors = array();
    
    foreach ($this->formatted_input as $key => $bags) {
      
      if ($key == $color || in_array($key, $this->checked_colors)) {
        continue;
      }
      ...
      foreach ($bags as $bag) {
        if ($bag['color'] == $color) {
          $matched_colors[] = $key; 
          $matched_colors = array_merge($matched_colors, $this->getBagsThatHoldColor($key));
          break;
        }
      }
    }
  ...
  function getBagsInsideColor($color, $number = 1) {
    $total = 0;
    ...
    foreach ($this->formatted_input as $key => $bags) {
      ...
      foreach($bags as $bag) {
        $line_total = $number * $bag['num'];
        $total += $line_total + $this->getBagsInsideColor($bag['color'], $line_total);
      }
    }
